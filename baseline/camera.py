...
class Camera:
    ...
    def from_json_parameters(self, calib_json_object):
        ...
        self.rotation = np.array([
            ...
            [np.sin(pan) * np.sin(tilt), -np.sin(tilt)
             * np.cos(pan), np.cos(tilt)]
        ], dtype='float')

        self.rotation = np.transpose(
            pan_tilt_roll_to_orientation(pan, tilt, roll))

        self.position = np.array(
            calib_json_object['position_meters'], dtype='float')
